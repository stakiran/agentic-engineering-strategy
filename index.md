...
- (ワークフローやフィードバックループまわり)
    - エージェンティック・トポロジー: Agentic Engineering における処理の主体（≒エージェント）をどう流すかを定義したもの、あるいはそのようなグラフ。関連する呼び方の例を右記する。グラフ、ネットワーク、チェーン、パイプライン、フィードバックループ、DAG
- (割り込みまわり)
    - インターベンション: 介入の意で、LLM 呼び出しまたはエージェンティック・トポロジーの処理に途中介入すること。例はタイムアウトや例外発生時の処理など。なお、しきい値を超えたときに break するといった条件分岐はエージェンティック・トポロジーとして組み込まれたロジックであり、インターベンションではない
