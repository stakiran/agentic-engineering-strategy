...
- Orchestration: 複数のエージェントをどう連携させるかを担う。**ローカルコンテキスト** と **エージェンティック・トポロジー** を持つ
...
- Orchestration: 複数のハーネスをどう連携させるかを担う。**ローカルコンテキスト** と **エージェンティック・トポロジー** を持つ
- Harness: エージェントをラップして **ガバナンス** を担う
- Agent: 複数のスキルを用いて何らかの仕事を行う。**ローカルコンテキスト** を含む
- Skill: 再利用可能な形で処理をパッケージングしたもの
- Tool: スキルの一部処理を担う。決定的。スキル内に閉じている
